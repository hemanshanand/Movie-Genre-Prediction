...
def predict():
    ...
    try:
        if request.method == "POST":
            ...
            input_tfidf = tfidf.transform([clean_string(my_string)])
            input_transformed_df = pd.DataFrame(input_tfidf.toarray(), columns=tfidf.get_feature_names())
            ...
            input_final_df = scaler.transform(input_final)

            input_pred = model.predict_proba(input_final_df)
